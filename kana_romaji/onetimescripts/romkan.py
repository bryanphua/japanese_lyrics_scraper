...
っす      ssu      っず      zzu      っせ      sse      っぜ      zze      っそ      sso
っぞ      zzo      った      tta      っだ      dda      っち      tchi
っちゃ      tcha      っちゅ      tchu      っちょ      tcho      っぢ      ddi
...
# Use Katakana
# all kunrei overwritten in KANROM
# ROMKAN both kunrei and hephburn
KANROM = {}
ROMKAN = {}
overwritten = []
...
ROMKAN.update({"du": "ヅ", "di": "ヂ", "fu": "フ", "ti": "チ",
               "wi": "ウィ", "we": "ウェ", "wo": "ヲ"})
#di du wo wi replaced w single kana except wi -> wa i
...
print "|".join(sorted(KANROM_H.keys(), key=cmp_to_key(_kanpat_cmp)))
...
def to_hepburn(str):
    ...
    tmp = str
    tmp = KANPAT.sub(lambda x: KANROM[x.group(0)], tmp)      #(kana|kana|sorted by length desc)
